...
public interface TRAN_MAIN_TRM_WRK_REP extends JpaRepository<TRAN_MAIN_TRM_WRK_ENTITY, String> {
	 ...
	 * + "GETDATE(), :netAmount, '2099-12-31', 'SYSTEM', GETDATE(), 'N')",
	 * nativeQuery = true) void insertNewAccountBalance(@Param("accountNum") String
	 * accountNum, @Param("netAmount") String netAmount);
	 * 
	 */

	/*
	 * @Query(value = "SELECT TOP 1 TRAN_DATE_BAL FROM BGLS_DAILY_ACCT_BAL " +
	 * "WHERE ACCT_NUM = :accountNum " + "ORDER BY TRAN_DATE DESC", nativeQuery =
	 * true) List<BigDecimal> findTRAN_DATE_BALByAccountNumber(@Param("accountNum")
	 * String accountNum);
	 * 
	 */
	...
	@Query(value = "SELECT TRAN_DATE_BAL FROM BGLS_DAILY_ACCT_BAL " + "WHERE ACCT_NUM = :accountNum "
			+ "AND TRAN_DATE = (SELECT MAX(TRAN_DATE) FROM BGLS_DAILY_ACCT_BAL WHERE ACCT_NUM = :accountNum)", nativeQuery = true)
	 ...
	 * "VALUES ('1000', 'ASSET', '1100', 'CASH AND BANK BALANCES', :accountNum, 'BANK ACCOUNT', 'SCR', "
	 * +
	 * "0.0000, 0.0000, :tranDateBal, GETDATE(), :netAmount, '2099-12-31', 'SYSTEM', GETDATE(), 'N')"
	 * , nativeQuery = true) void insertNewAccountBalance(@Param("accountNum")
	 * String accountNum,
	 * 
	...
	@Query(value = "INSERT INTO BGLS_DAILY_ACCT_BAL "
			...
			+ ":totalDebit, :totalCredit, :tranDateBal, :tranDate, :netAmount, '2099-12-31', 'SYSTEM', GETDATE(), 'N')", nativeQuery = true)
	...
	@Query(value = "INSERT INTO BGLS_DAILY_ACCT_BAL "
			...
			+ "ENTRY_USER_ID, ENTRY_TIME, DEL_FLG) "
			+ "VALUES (:glCode, :glDesc, :glshCode, :glshDesc, :accountNum, :acctName, :acctCrncy, "
			+ ":totalDebit, :totalCredit, :tranDateBal, DATEADD(day, 3, GETDATE()), :netAmount, '2099-12-31', 'SYSTEM', GETDATE(), 'N')", nativeQuery = true)
	...
	@Query(value = "SELECT acct_num, " + "acct_name, " + // Include acct_name
			...
			+ "AND CAST(TRAN_DATE AS DATE) = CAST(:tranDate AS DATE) " + "GROUP BY acct_num, acct_name", // Group by
																											// acct_name
																											// as well
			nativeQuery = true)
	...
	@Query(value = "SELECT SUM(COUNTAMT) AS TOTAL_COUNT " + "FROM ( " + "    SELECT COUNT(TRAN_AMT) AS COUNTAMT "
			+ "    FROM BGLS_TRM_WRK_TRANSACTIONS " + "    WHERE DEL_FLG != 'Y' " + "    AND TRAN_STATUS = 'POSTED' "
			+ "    GROUP BY acct_num " + ") AS AccountCounts", nativeQuery = true)
	...
	@Query(value = "SELECT TRAN_DATE_BAL FROM BGLS_TRM_WRK_TRANSACTIONS " + "WHERE ACCT_NUM = ?1 "
			+ "AND TRAN_DATE = DATEADD(day, -1, CAST(?2 AS DATE))", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM BGLS_TRM_WRK_TRANSACTIONS " + "WHERE CAST(TRAN_DATE AS DATE) = :tranDate "
			...
			+ "AND PART_TRAN_TYPE = 'Debit'", nativeQuery = true)
